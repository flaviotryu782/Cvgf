...
public class PlayerController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (animationController != null) animationController.SetLocomotion(direction.magnitude * (sprint ? 1f : .65f), sprint && direction.sqrMagnitude > .01f);
        ...
        if (MobileInput.Instance != null) kick = MobileInput.Instance.ConsumeKick(out charge, out type);
        else if (Input.GetKeyDown(KeyCode.Space)) kick = true;
        if (ballController.Owner != transform) return;
        if (pass) { animationController?.PlayPass(); ballController.Pass(GetPassDirection(), passPower); }
        else if (kick) { animationController?.PlayKick(); ballController.Shoot((enemyGoal.position - ball.position).normalized, shotPower, type, charge); }
    }

    private Vector3 GetPassDirection()
    {
        Transform best = null; float bestScore = float.MaxValue;
        if (passTargets != null) foreach (Transform target in passTargets)
        {
            if (target == null) continue;
            Vector3 delta = target.position - transform.position; float score = delta.magnitude - Vector3.Dot(transform.forward, delta.normalized) * 3f;
