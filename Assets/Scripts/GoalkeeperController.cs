...
public class GoalkeeperController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (rb != null && Vector3.Distance(transform.position, ball.position) < saveDistance && Time.time >= nextSaveTime)
        {
            nextSaveTime = Time.time + .6f;
            animationController?.PlayDive();
            animationController?.PlaySave();
            AudioManager.Instance?.PlaySave();
            rb.AddForce((ball.position - transform.position).normalized * 7f, ForceMode.Impulse);
        }
    }

    public void PlayPenaltySave() { animationController?.PlayDive(); animationController?.PlaySave(); }
}
